    ...
    var AutomapperAsync = function() {
        ...
        this.mapPath = function(path, options, callback) {
            ...
            if (_.has(options, 'isParsingFiles') && 
                ...
                async.waterfall([
                    function(cb) {
                        self.getJsFiles(path, options, function(err, fileInfo) {
                            if (err !== null) {
                                cb(err);
                            } else {
                                cb(null, fileInfo);
        ...
        this.getJsFiles = function(startPath, options, callback) {
            // check options
            var isIncludingNodeModules = false;
